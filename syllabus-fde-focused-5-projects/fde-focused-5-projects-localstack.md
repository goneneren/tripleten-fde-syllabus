# AI Forward Deployed Engineer — Five-Project LocalStack Program
...
Projects 1–4 and Phase 5.A run locally with Docker Compose. LocalStack Hobby replaces selected infrastructure containers beginning in Project 2; students use individually owned accounts for personal, non-production educational work. These local stages do not require a real AWS account or expose a public endpoint. Phase 5.B first passes the same local acceptance path and then deploys one temporary, protected endpoint in a course-managed AWS sandbox. The total per-student program allowance remains **$20 for approved LLM APIs and $180 for AWS**.
...
- **Cloud transition:** Phase 5.B replaces local endpoint URLs and sentinel credentials with supplied real-AWS configuration and adapters; students do not build a general cloud platform.
...
- Projects 2–4 and Phase 5.A use only `awslocal`; plain AWS CLI is not used in the local stages. Compose injects sentinel credentials and region values, the shared client factory injects the local endpoint, and a CI gate rejects local-stage SDK clients constructed outside that factory. Phase 5.B uses the real AWS CLI and supplied AWS adapters, and the local-endpoint CI gate does not apply to its real-AWS configuration. Host AWS credential directories are never mounted.
