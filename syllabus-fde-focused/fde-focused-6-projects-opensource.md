...
Projects 1–5 run locally with Docker Compose and an open-source stack. They do not require a cloud account or expose a public endpoint. Project 6 first passes the same local acceptance path and then deploys one temporary, protected endpoint in a course-managed AWS sandbox. The total per-student program allowance remains **$20 for approved LLM APIs and $180 for AWS**.
...
- **Search and retrieval:** PostgreSQL/pgvector and OpenSearch BM25.
- **Messaging:** RabbitMQ with bounded retry and dead-letter patterns.
- **Identity and secrets:** development-mode Keycloak and OpenBao.
- **AI:** LangGraph, Ollama, optional local vLLM, Ragas, Arize Phoenix.
- **Observability and security:** OpenTelemetry, Prometheus, Grafana, Jaeger or Tempo, Semgrep, Trivy, and Gitleaks.
- **Cloud transition:** supplied AWS templates and adapters in Project 6; students are not asked to build a general cloud platform.
...
- Open-source services teach portable interfaces and operational reasoning. Students must still explain the limits of translating them to managed AWS services.
...
- **Local environment:** Docker Compose, FastAPI, PostgreSQL, Redis, OpenTelemetry, Prometheus, Grafana, and Jaeger or Tempo.
...
- **[Tools]** Docker Compose, FastAPI, PostgreSQL, Redis, OpenTelemetry, Prometheus, Grafana, Jaeger or Tempo.
...
- **[Tools]** Pytest, k6, OpenTelemetry, Prometheus, Grafana, Jaeger or Tempo, deterministic provider emulator.
...
- **Local environment:** Docker Compose, MinIO, PostgreSQL/pgvector, OpenSearch, supplied document parser, local embeddings, and a supplied cross-encoder.
...
- **[Tools]** PostgreSQL/pgvector, OpenSearch BM25, supplied local cross-encoder, FastAPI, Alembic.
- Combine pgvector dense retrieval with OpenSearch keyword retrieval.
...
- **Local environment:** Docker Compose, Ollama or optional local vLLM, provider emulator, RabbitMQ, OpenTelemetry, Prometheus, and Grafana.
...
- **[Tools]** RabbitMQ, provider emulator, Tenacity or supplied resilience library, Pytest, OpenTelemetry.
...
- Use RabbitMQ and a dead-letter path only where the selected workflow requires asynchronous recovery.
...
- **Local environment:** Keycloak in development mode, OpenBao in development mode, Semgrep, Trivy, Gitleaks, OpenTelemetry, and supplied attack scenarios.
...
- **[Tools]** Keycloak, OpenBao, FastAPI, Presidio or supplied redactor, Semgrep, Trivy, Gitleaks, Pytest.
...
- Configure one secrets improvement and one deterministic CI security gate.
...
- **[Tools]** Ragas, Arize Phoenix, OpenTelemetry, Pytest, cached fixtures, bounded LLM-as-judge worksheet.
...
- Replace local infrastructure adapters with supplied AWS bindings; do not rebuild application logic.
